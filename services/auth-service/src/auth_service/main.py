...
from auth_service.controllers import auth_router
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.exception("event bus connect failed; service will continue without it")
...
def create_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(
        title="Terramind — Auth Service",
        description="Registro, login, JWT e RBAC para o Terramind.",
        version="0.1.0",
        docs_url="/auth/docs",
    ...
    @app.get("/auth/health", tags=["health"], summary="Liveness probe")
